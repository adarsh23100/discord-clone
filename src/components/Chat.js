...
function Chat() {
  ...
  return (
    <div className="flex flex-col h-screen">
        ...
        <main className="flex-grow overflow-y-scroll scrollber-hide">
            {messages?.docs.map((doc) => {
                const { message, timestamp, name, photoURL, email } = doc.data();

                return <Message key={doc.id} id={doc.id} message={message} timestamp={timestamp} name={name} email={email} photoURL={photoURL}/>;
            })}
        ...
        <div className="flex items-center p-2.5 bg-[#40444b] mx-5 mb-7 rounded-lg">
            ...
            <form className="flex-grow">
                ...
                className="bg-transparent focus:outline-none text-[#dcddde] w-full placeholder-[#72787d] text-sm" 
                ref={inputRef} />
